...
(function ($) {
	...
	$(".qtybutton").on("click", function () {
	    var cart_form = $('.cart-plus-minus')
	    var $button = $(this);
	    if ($button.closest('form').find('button').length == 1){
            ...
            if ($button.text() == "+") {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.err) {
                            $(cart_form).attr('active', false)
                            $(cart_form).attr('data-tooltip', response.err)
                        }
                        else {
                            $(cart_form).attr('active', true)
                            $button.parent().find("input").val(newVal);
                            $('#cartSubTotalPrice span').html('$' + response.cart_subtotal)
                            $('#cartTotalPrice').html('$' + response.cart_total)
                            $('#' + response.id).html('$' + response.product_total_price)
                            if (response.get_discount) {
                                $('#cartDiscount').html('$' + response.get_discount)
                            }
                        }
